...
try {
    ...
    $stmt = $db->prepare("
        INSERT INTO facebook_tokens (user_id, access_token, refresh_token, page_id, page_access_token, expires_at) 
        VALUES (?, ?, ?, ?, ?, ?) 
    ...
    do {
        ...
        if (isset($pages_data['data'])) {
            foreach ($pages_data['data'] as $page) {
                ...
                try {
                    $stmt->execute([1, $access_token, $refresh_token, $page_id, $page_access_token, $expires_at]);
                    $page_count++;
    ...
    header("Location: /superadmin/myapicontainer/create-api-view/manage_apis");
    exit;
