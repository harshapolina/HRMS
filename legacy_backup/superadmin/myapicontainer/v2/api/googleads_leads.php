<?php
// google_api.php - robust lead ingestion with round-robin assignment + error handling
...
// constants (keep your real key here)
$CRM_API_KEY_SERVER = "533f4175837e145064605e15e12c7273f98746fec7459b0168af9394a22c6efab6bba75cce18a3555250e473f4907d22aaae3e3f12e46dd8ef22fac38737c537"; // truncated in example - keep original
...
// blocked IP check (if location used for ip)
if (!empty($location)) {
...
// --- Round-robin: choose assigned_user with phone; skip users missing phone ---
// We'll stop and return error if no valid assigned user exists.
$assign_users_raw = $project['assign_user'] ?? '';
$assign_users = array_filter(array_map('trim', explode(',', $assign_users_raw)), fn($v) => $v !== '');
$assign_users = array_values($assign_users); // reindex
$last_assigned_user = $project['last_assigned_user'] ?? null;
...
if ($totalAssignUsers > 0) {
    for ($i = 0; $i < $totalAssignUsers; $i++) {
        ...
        try {
            ...
            file_put_contents("assign_user_error.log", date("Y-m-d H:i:s") . " | DB error when fetching $candidate: " . $e->getMessage() . PHP_EOL, FILE_APPEND);
            continue;
        }
        ...
        $next_user_index = $idx; // index of assigned user
        break;
    }
}
...
$debug_info = [
    ...
    'last_assigned_user' => $last_assigned_user,
    'tried_users' => $tried_users,
    'skipped_users' => $skipped_users,
    'assigned_user' => $assigned_user,
    'fetched_user_row' => $fetched_user_row
];

// If no assigned user found -> stop insertion and return error
if (!$assigned_user) {
    http_response_code(500);
    $debug_info['error'] = "No valid assigned user with phone found. Insertion aborted.";
    echo json_encode([
        "status" => "error",
        "message" => "No assigned user with a valid phone number found. Lead insertion aborted.",
        "debug_info" => $debug_info
    ]);
    exit;
}

// Normalize the assigned user's phone (if you need to send SMS)
$normalized_assigned_user_number = normalizePhoneNumber($user_number);

// Proceed to DB insertion with transaction
try {
    $db->beginTransaction();
    ...
    if ($checkStmt->rowCount() > 0) {
        ...
        $shouldNotify = false;
        $db->commit();
        echo json_encode(["status" => "duplicate", "message" => "Duplicate lead — lead_count incremented."]);
        exit;
    }

    // Insert new lead using assigned_user
    ...
    // Update round-robin last_assigned_user now that insert succeeded
...
// Notifications - only if allowed
$notifyResult = null;
$fast2smsResult = null;

if ($shouldNotify) {
    // build notify payload (user_codes should contain assigned_user)
    try {
    ...
    try {
        ...
        // reuse insertNotificationAndReceipts inline to avoid code duplication
        $db->beginTransaction();
    ...
    // Fast2SMS - send to normalized_assigned_user_number only if present
    try {
