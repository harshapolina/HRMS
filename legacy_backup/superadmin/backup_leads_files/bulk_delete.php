...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['row_ids'])) {
    ...
    if (!empty($rowIds)) {
        ...
        if ($stmt->execute()) {
            ...
            header("Location: /superadmin/upload_data?status=Deleted successfully");
            exit();
        } else {
            // Redirect with an error message
            header("Location: /superadmin/upload_data?status=Error deleting records");
            exit();
        }
    } else {
        // Redirect if no rows were selected
        header("Location: /superadmin/upload_data?status=No rows selected for deletion");
        exit();
    }
} else {
    // Redirect if the request method is not POST or no row_ids[] are sent
    header("Location: /superadmin/upload_data?status=Invalid request");
    exit();
}
