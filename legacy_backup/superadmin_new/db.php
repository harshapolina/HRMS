<?php
  ...
  class Database extends Config {
    ...
    $ccrevenue, $cstatus, $brecived, $leadsource, $bremarks, $filePathStored) {
      ...
      update_date_column, source_lead, remarks, document_path) VALUES (:bdate, :bmonth, :developer, :bproject, :cname, :cnumber, :cemail, 
      :tproject, :unitno, :psize, :cagreement, :ccashback, :crevenue, :cccashback, :ccrevenue, :cstatus, :brecived, NOW(), :leadsource, :bremarks, :document_path)';
    ...
    public function read() {
      // Query to select all rows from updaterowtable, and non-overlapping rows from admintable
      $sql = "
          SELECT u.* 
          FROM updaterowtable u
          UNION
          SELECT a.* 
          FROM admintable a
          LEFT JOIN updaterowtable u ON a.id = u.id
          WHERE u.id IS NULL
          ORDER BY id DESC
      ";
      $stmt = $this->conn->prepare($sql);
    ...
    public function status_counter($groupKey) {
      ...
      // Prepare the SQL query to count status occurrences for the given financial year
      $sql = "SELECT 
                  COALESCE(SUM(CASE WHEN astatus = 'Received' THEN 1 ELSE 0 END), 0) AS received_count,
                  COALESCE(SUM(CASE WHEN astatus = 'Cancled' THEN 1 ELSE 0 END), 0) AS canceled_count,
                  COALESCE(SUM(CASE WHEN astatus = 'Processing' THEN 1 ELSE 0 END), 0) AS processing_count
              FROM admintable
              WHERE booking_month BETWEEN :start_month AND :end_month";
  ...
  public function update(
    ...
    if ($updateInUserTable) {
        ...
        $stmtInsertOrUpdateReferenceTable->execute([
            'id' => $id,
            'bdate' => $bdate,
            'bmonth' => $bmonth,
            'developer' => $developer,
