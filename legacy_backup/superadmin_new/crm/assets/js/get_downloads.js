(function() {
  const basePath = '/superadmin_new/crm'; // adjust if your endpoints live elsewhere
  const requestUrl = (path) => `${basePath}/${path}`;
...
function getCurrentFilters() {
  ...
  function qsFromParams(params){
    ...
    for (const k in params) {
      ...
      if (k === 'multiFilters') {
        qp.append(k, JSON.stringify(params[k])); // always send JSON string
      } else {
    ...
    return new Promise((resolve, reject) => {
      ...
      async function tick(){
        try{
          ...
          if (j.status === 'failed') return reject(new Error('Server export failed'));
  ...
  downloadBtn.addEventListener('click', async function(ev){
    ...
    // If the button already has a ready URL, use it (user-gesture)
    const readyUrl = downloadBtn.dataset.downloadUrl;
    if (readyUrl) {
    ...
    try {
      ...
      // Poll until job done. When done, the UI will show "Download Ready" so the user must click to actually download.
      ...
      // set button to ready state (user must click to trigger browser download)
      downloadBtn.dataset.downloadUrl = result.downloadUrl;
      downloadBtn.dataset.state = 'ready';
      downloadBtn.innerHTML = '<i class="bi bi-download"></i> Download Ready — Click to download';
      setStatus('Export ready. Click the button to start download.', false);
      enableButton();

    } catch (err) {
      console.error(err);
      downloadBtn.dataset.state = '';
      downloadBtn.innerHTML = '<i class="bi bi-download"></i> Download Leads';
