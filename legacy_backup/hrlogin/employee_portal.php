...
$status = 'Not Logged';
$today_date = null;
$stmt = $con->prepare('SELECT today_date, today_punch_in, today_punch_out, today_status FROM user_attendance WHERE user_id = ?');
$stmt->bind_param('i', $user_id);
$stmt->execute();
$stmt->store_result();
if ($stmt->num_rows > 0) {
    $stmt->bind_result($today_date, $punch_in_time, $punch_out_time, $status);
    $stmt->fetch();
}
$stmt->close();
...
if ($today_date !== $today) {
    ...
    $punch_out_time = null;
    $status = 'Not Logged';
    $con->query("UPDATE user_attendance SET today_date = NULL, today_punch_in = NULL, today_punch_out = NULL, today_status = NULL, today_lat_in = NULL, today_lng_in = NULL, today_lat_out = NULL, today_lng_out = NULL, today_ip = NULL, today_total_hours = NULL WHERE user_id = $user_id");
}
...
<head>
    ...
    <script src="assets/js/geolocation.js?v=<?php echo time(); ?>"></script>
    <style>
        :root {
            --primary-teal: #227477;
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        if (document.getElementById('punchOutBtn')) {
            ...
            const poForm = document.getElementById('punchOutForm');
            ...
            poForm.addEventListener('submit', (e) => {
                ...
                if (btn) {
                    ...
                    btn.innerHTML = '<i class="bi bi-hourglass-split"></i> Punching Out...';
                }
                stopLiveTracking(<?php echo json_encode($_SESSION['username'] ?? 'Employee'); ?>, () => {
                    try {
                        HTMLFormElement.prototype.submit.call(poForm);
                    } catch (submitErr) {
                        console.error("Standard submit failed, trying fallback:", submitErr);
                        poForm.submit();
                    }
                });
            });
