...
if (!file_exists(__DIR__ . '/vendor/autoload.php')) {
    echo json_encode(["status" => "error", "message" => "Mail libraries are not installed. Run composer install in the hrlogin folder."]);
    exit;
}
...
$offer = $db->getOfferLetter($offer_id);
if (!$offer) {
...
if (empty($to) || !filter_var($to, FILTER_VALIDATE_EMAIL)) {
    echo json_encode(["status" => "error", "message" => "A valid candidate email address is required."]);
    exit;
}

// Check if user has custom HTML saved in user_documents
$content = '';
if (!empty($offer['user_id'])) {
    try {
        $stmt = $conn->prepare("SELECT content FROM user_documents WHERE user_id = :uid AND document_type = 'offer_letter'");
        $stmt->execute(['uid' => $offer['user_id']]);
...
// Fallback to default template if no custom content
if (empty(trim(strip_tags($content)))) {
    $content = $db->getDefaultOfferLetterHtml($offer);
    if (!empty($offer['user_id'])) {
        try {
            $checkSql = "SELECT id FROM user_documents WHERE user_id = :uid AND document_type = 'offer_letter' LIMIT 1";
            $checkStmt = $conn->prepare($checkSql);
            $checkStmt->execute(['uid' => $offer['user_id']]);
            $existingDoc = $checkStmt->fetch(PDO::FETCH_ASSOC);
            if ($existingDoc) {
                $updateDocSql = "UPDATE user_documents SET content = :content WHERE id = :id";
                $updateDocStmt = $conn->prepare($updateDocSql);
                $updateDocStmt->execute(['content' => $content, 'id' => $existingDoc['id']]);
            } else {
                ...
                $insertDocStmt->execute(['uid' => $offer['user_id'], 'content' => $content]);
            }
        } catch (Exception $e) {
            // ignore — email can still use generated HTML
        }
    }
}

if (empty(trim(strip_tags($content)))) {
    echo json_encode(["status" => "error", "message" => "Offer letter content is empty. Please regenerate the offer letter."]);
    exit;
}

// Reuse helper functions from mail_letter.php for styling, logo, headers, footers
...
$message = "<html><body>";
$message .= "<p>Dear " . htmlspecialchars($candidateName) . ",</p>";
$message .= "<p>Please find your Offer Letter attached below as a PDF document.</p>";
...
$gmail_username = trim($settings['gmail_username'] ?? '');
$gmail_app_password = trim(str_replace(' ', '', $settings['gmail_app_password'] ?? ''));
...
try {
    ...
    $mail->addAddress($to, $candidateName);
    ...
    try {
        ...
        $pdfExtraStyles = getOfferLetterDompdfStyles();
        $pdfHtml = "
        <html>
        <head>
            <style>
        ...
        $pdfFilename = "Offer_Letter_" . str_replace(' ', '_', $candidateName) . ".pdf";
        $mail->addStringAttachment($pdfOutput, $pdfFilename);
    } catch (Throwable $pdfError) {
        error_log('Offer letter PDF generation failed: ' . $pdfError->getMessage());
    }

    $mail->send();

    try {
        $emailedBy = $_SESSION['username'] ?? $_SESSION['useremail'] ?? $_SESSION['role'] ?? '';
        $db->markOfferLetterEmailed($offer_id, $emailedBy);
    } catch (Exception $e) {
        // ignore
    }
    ...
    $detail = (isset($mail) && !empty($mail->ErrorInfo)) ? $mail->ErrorInfo : $e->getMessage();
    echo json_encode(["status" => "error", "message" => "Message could not be sent. " . $detail]);
}
?>
